...
<div class={twMerge('flex flex-row gap-3 px-3 my-2', self && 'flex-row-reverse')}>
	<div class="place-self-end w-8">
		<img class="min-w-[2rem] min-h-[2rem] max-w-full rounded-full shadow" src={avatarSrc} alt="" />
	</div>
	<div class="max-w-xl">
		<p class={twMerge('text-left pb-1 text-xs', self ? 'text-right' : 'text-left')}>
